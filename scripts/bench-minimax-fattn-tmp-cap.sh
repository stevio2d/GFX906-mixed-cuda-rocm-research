...
CTX=${CTX:-69376}
PORT_BASE=${PORT_BASE:-21340}
RUN_DIR=${RUN_DIR:-$ROOT/bench-results/minimax-fattn-tmp-cap-$(date -u +%Y%m%dT%H%M%SZ)}
...
printf "name\tstatus\tprompt_n\tpredicted_n\tprompt_per_second\tpredicted_per_second\tprompt_ms\tpredicted_ms\tserver_log\tresponse_file\n" > "$RESULTS_TSV"
...
runs=(
    ...
    "plain_ub454|454|"
    "plain_ub456|456|"
    "tmpcap0_ub448|448|--cuda-fattn-vec-tmp-headroom-mib 0"
    "tmpcap128_ub452|452|--cuda-fattn-vec-tmp-headroom-mib 128"
    "tmpcap0_ub452|452|--cuda-fattn-vec-tmp-headroom-mib 0"
    "tmpcap0_ub450|450|--cuda-fattn-vec-tmp-headroom-mib 0"
    "tmpcap0_ub454|454|--cuda-fattn-vec-tmp-headroom-mib 0"
    "tmpcap256_ub456|456|--cuda-fattn-vec-tmp-headroom-mib 256"
    "tmpcap0_ub456|456|--cuda-fattn-vec-tmp-headroom-mib 0"
)
...
with open(results_tsv, "a", encoding="utf-8") as f:
    f.write(
        ...
        f"{prompt_ms}\t{predicted_ms}\t{log_file}\t{response_file}\n"
    )
PY
}
...
for i in "${!runs[@]}"; do
    ...
    cleanup_server "$srv_pid"
    rg -n "common_apply_cuda_fattn_env|layer placement summary|backend=.*layers=|placement_policy=|out of memory|failed|fallback|sched_reserve|vector temp cap" "$log_file" > "$RUN_DIR/${name}.summary.txt" || true
    sleep 3
done
